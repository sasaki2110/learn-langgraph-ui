...
2. **LangSmith Studioにアクセス**
   - ブラウザで以下のURLにアクセス：
     ...
     ```
   - **重要**: 
     - `http://localhost:2024`ではなく、`http://127.0.0.1:2024`を使用してください
     - `http://127.0.1:2024`（誤字）ではなく、`http://127.0.0.1:2024`（正しい）を使用してください
   - **CORSエラーが発生する場合**: `langgraph.json`に`allowed_origins`が設定されているか確認してください
